...
export default function LoginScreen({navigation}) {
  ...
  useEffect(() => {
    const f = async () => {
      try {
        const data = await AsyncStorage.getItem("@isremembered");
          if (data !== null) {
            const g = async () => {
              try {
                const data2 = await AsyncStorage.getItem("@sessiondata");
                  if (data2 !== null) {
